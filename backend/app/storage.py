...
class EventStore:
    ...
    def get_user_by_token(self, token: str) -> dict[str, str] | None:
        with self._connect() as connection:
            ...
            user = connection.execute(
                f"SELECT u.id, u.email, u.name, u.photo, u.photo_gallery, u.instagram, u.school, u.year, u.major, u.hometown, u.bio, u.hobbies FROM users u JOIN sessions s ON s.user_id = u.id WHERE s.token = ? AND {expiration_clause}",
                (token,),
            ).fetchone()
    ...
    def list_profiles(self) -> list[dict[str, Any]]:
        with self._connect() as connection:
            rows = connection.execute("SELECT id, name, photo, photo_gallery, instagram, school, year, major, hometown, bio, hobbies FROM users ORDER BY name").fetchall()
        return [self._profile(row) for row in rows]

    def get_profile(self, user_id: str) -> dict[str, Any] | None:
        with self._connect() as connection:
            row = connection.execute("SELECT id, name, photo, photo_gallery, instagram, school, year, major, hometown, bio, hobbies FROM users WHERE id = ?", (user_id,)).fetchone()
        return self._profile(row) if row else None

    def update_profile(self, user_id: str, profile: dict[str, Any]) -> dict[str, Any] | None:
        fields = ("name", "photo", "photo_gallery", "instagram", "school", "year", "major", "hometown", "bio", "hobbies")
        ...
        with self._connect() as connection:
            cursor = connection.execute("UPDATE users SET name = ?, photo = ?, photo_gallery = ?, instagram = ?, school = ?, year = ?, major = ?, hometown = ?, bio = ?, hobbies = ? WHERE id = ?", (*[values[field] for field in fields], user_id))
